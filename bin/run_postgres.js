...
function load_data(callback){
    ...
    client.connect(function(err){
        if(err){
            return callback(new Error("Couldn't connect to Postgres db: " + err.message));
        }
        ...
        client.query("SELECT settings_json FROM settings WHERE id = 1", function(err, result){
            ...
            try {
                ...
                return callback("Found results in postgres table, but failed to parse: " + err);
            }
        });
    });
}
...
function createTableIfNeeded(client, callback){
    client.query("SELECT * FROM settings LIMIT 1", function(err, result){
        if(err || result.rows.length == 0) {
            return client.query("CREATE TABLE settings (id INTEGER, settings_json JSON )", callback);
        } else {
            // table exists
...
function save_data(data, callback){
    ...
    client.connect(function(err){
        ...
        createTableIfNeeded(client, function (err){
            ...
            var updateQuery = "UPDATE settings SET settings_json='" + JSON.stringify(data) + "' WHERE id = 1";
            client.query(updateQuery, function(err, result){               
               ...
               if(result.rowCount == 0){
                    var insertQuery = "INSERT INTO settings VALUES (1, '" + JSON.stringify(data) + "')";
