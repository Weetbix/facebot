...
function findFBUser(api, search_str, allowNonFriends) {
    var userID;
    var promise;
    if (/^\d+$/.test(search_str)) {
        //if all numbers assume it is the userid
        userID = search_str;
        promise = Q.nfcall(api.getUserInfo, userID);
    } else {
        promise = getFBUserIDByName(api, search_str).then(id => {
            userID = id;
            return Q.nfcall(api.getUserInfo, id);
        });
    }
...
function getFBUserIDByName(api, name) {
    return Q.nfcall(api.getUserID, name).then(data => {
