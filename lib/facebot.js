...
Facebot.prototype.setupUsers = function(){
    ...
    return Q.all(usernames.map(findUser))
            .spread(function(bot, auth){
                self.user = bot;
                self.authorised_user = auth;        
    });
};
...
Facebot.prototype.setupFacebook = function(){
    ...
    .then(function(data){
       if(self.settings.debug_messages)
            self.DMAuthorisedUser("Loaded data, found " + data.channelLinks.length + " channel links.");
           
        // Load the linked channels
        self.channelLinks = data.channelLinks;

        return self.createFBApi(data);
    })
    .fail(function(err){
        if(self.settings.debug_messages)
            self.DMAuthorisedUser("Couldn't log in with any saved data, logging in with email and pass ("+err+").");
        ...
        return self.createFBApi(facebookConfig);
    })
    .then(function(){        
        self.saveData();
    });
...
Facebot.prototype.createFBApi = function(credentials){
    ...
    return Q.nfcall(facebook, credentials)
           .then(function(api){
               if(self.settings.debug_messages)
                   self.DMAuthorisedUser("Logged into facebook")
               ...
               api.setOptions({
                ...
                api.listen(function (err, fbmessage){ 
                    if(!err)
                        self.postFBMessagesToSlack(fbmessage);
                });
           });
}
...
Facebot.prototype.postFBMessagesToSlack = function(fbmessage){
    ...
    .forEach(function(link){        
        ...
        self.handleFBAttachments(fbmessage, link);
    });
}
...
Facebot.prototype.handleFBAttachments = function(fbmessage, link){
    ...
    fbmessage.attachments.forEach(function(attachment){
        ...
        {
            case "sticker": self.handleFBImageMessages(attachment.url, link); break;
            case "photo": self.handleFBImageMessages(attachment.hiresUrl, link); break;
            case "animated_image": self.handleFBImageMessages(attachment.rawGifImage, link); break;
            ...
            case "file":
                if(S(attachment.name).startsWith("audioclip"))
                    self.handleFBAudioMessages(attachment, link);
                break;
            case "video":
                self.handleFBVideoMessages(attachment, link);
                break;    
        }
    });
}
...
// Sends a (slack) direct message to the authorised user
Facebot.prototype.DMAuthorisedUser = function(message){
