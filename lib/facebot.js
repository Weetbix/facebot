...
const Facebot = function Constructor(settings, load_data, save_data) {
...
Facebot.prototype.onStart = async function() {
...
Facebot.prototype.setupUsers = async function() {
    ...
    this.authorised_user = await findUser(this.settings.authorised_username);
};

// Attempts to log into facebook
Facebot.prototype.setupFacebook = async function() {
    try {
...
Facebot.prototype.createFBApi = async function(credentials) {
...
Facebot.prototype.handleFBNotification = function(fbmessage) {
    ...
    if (fbmessage.type == 'message') threadID = fbmessage.threadID.toString();
    if (fbmessage.type == 'typ') threadID = fbmessage.from.toString();

    if (!threadID) return;

    this.channelLinks
        .filter(link => link.fb_name === threadID)
        .forEach(link => {
...
Facebot.prototype.handleFBAttachments = function(fbmessage, link) {
    fbmessage.attachments.forEach(attachment => {
        ...
        switch (attachment.type) {
            ...
            case 'photo': {
                const url = attachment.hiresUrl || attachment.largePreviewUrl;
...
Facebot.prototype.respondToCreateChatMessages = async function(message) {
    try {
        ...
        const friendname = message.text
            ...
            .trim();
        const friend = await fbUtil.findFBUser(this.facebookApi, friendname);
...
Facebot.prototype.respondToFriendSearchCommands = async function(message) {
...
// message came from
Facebot.prototype.postListOfLinkedChannels = async function(message) {
...
Facebot.prototype.postGroupJoinedMessage = async function(message) {
...
Facebot.prototype.groupUsersOnlyContains = async function(channelid, userids) {
    try {
